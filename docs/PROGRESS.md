...
| 9 | IN_PROGRESS | Execution backend done: lib/execution/{templates,message,execute}.ts, lib/claude/generate-message.ts, lib/razorpay/client.ts (real test-mode Payment Links REST call, notify disabled so Razorpay never touches the synthetic phone/email), Rule 6 auto-insert-notice flow wired into lib/batch/run-batch.ts. Verified against real batch: messages render via fallback template (no Anthropic key yet), the deliberate Rule-9 naive-template demo subset is correctly blocked from dispatch. Remaining: 9.2 needs a live Razorpay key to verify a real link end-to-end; 9.3 (pre-rendered Hinglish MP3) blocked on a TTS credential Phase 0 never listed — see docs/BLOCKERS.md, browser SpeechSynthesis fallback works today. UI (chat-bubble rendering, case detail page) still to come in Phase 10. |
| 10 | NOT_STARTED | |
| 11 | NOT_STARTED | |
| 12 | NOT_STARTED | |
| 13 | NOT_STARTED | |
