...
// try Claude (respecting Phase 6.2's never-throw contract), fall back to a
...
export type MessageSource = "claude" | "fallback_template" | "deliberate_naive_demo";
...
export type GenerateAndCheckMessageParams = {
   ...
   * Claude — used ONLY for the small seeded Rule-9 demonstration subset. */
...
export async function generateAndCheckMessage(params: GenerateAndCheckMessageParams): Promise<MessageOutcome> {
  ...
  if (params.useDeliberateNaiveTemplate) {
    ...
    const claudeResult = await generateMessage({
